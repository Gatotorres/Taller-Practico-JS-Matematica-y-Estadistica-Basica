...
function createBarChart(data) {
    ...
    canvas.width = 800; // Aumentar el tamaño del lienzo
    canvas.height = 400;
    ...
    // Dibujar las barras
    const barWidth = 30; // Aumentar el ancho de las barras
    let x = 50;
    let userIndex = 1; // Variable para numerar a cada usuario
    for (let i = 0; i < data.length; i++) {
        const trabajos = data[i].trabajos;
        for (let j = 0; j < trabajos.length; j++) {
            const salario = trabajos[j].salario;
            const barHeight = salario / 20; // Ajustar la escala
            ctx.fillStyle = 'green';
            ctx.fillRect(x, canvas.height - barHeight - 30, barWidth, barHeight); 
            // Ajustar la posición vertical
            ctx.fillStyle = 'black';
            ctx.fillText("Usuario " + userIndex, x, canvas.height - 10); 
            // Numerar cada usuario
            ctx.fillText("$" + salario, x, canvas.height - barHeight - 35); 
            // Agregar el texto del salario
            ctx.fillText(trabajos[j].year, x, canvas.height + 20); 
            // Ajustar posición del año
            x += 50; // Espacio entre barras
            userIndex++; // Incrementar el índice del usuario
        }
        x += 10; // Espacio entre grupos de barras
    }
}

// Llamar a la función para crear el gráfico de barras con los datos proporcionados
createBarChart(salarios);
};
*/